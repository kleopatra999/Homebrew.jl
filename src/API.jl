...
Retuns a list of all installed packages that are out of date as a `Vector{BrewPkg}``
"""
function outdated()
...
intsall from mainline Homebrew.
"""
function upgrade()
...
function deps(name::AbstractString)
    ...
    return info(String[dep for dep in obj["dependencies"]])
end
